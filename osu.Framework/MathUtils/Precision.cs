...
namespace osu.Framework.MathUtils
{
    ...
    {
        public static bool CheckEqualityWithErrors(float value1, float value2, float relativeError, float absoluteError)
        {
            if (value1 == value2) return true;
            float diff = Math.Abs(value1 - value2);
            return diff < absoluteError || diff < relativeError * (Math.Abs(value1) + Math.Abs(value2));
        }

        public static bool CheckEqualityWithErrors(double value1, double value2, double relativeError, double absoluteError)
        {
            if (value1 == value2) return true;
        ...
        public static bool AlmostEquals(Vector2 value1, Vector2 value2, float acceptableDifference = FLOAT_EPSILON) =>
            AlmostEquals(value1.X, value2.X, acceptableDifference) && AlmostEquals(value1.Y, value2.Y, acceptableDifference);
    }
}
