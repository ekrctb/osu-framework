...
namespace osu.Framework.Graphics.UserInterface
{
    public class TextBox : TabbableContainer, IHasCurrentValue<string>
    {
        ...
        protected override bool OnKeyDown(KeyDownEventArgs args)
        {
            ...
            if (args.State.Keyboard.ControlPressed || args.State.Keyboard.SuperPressed)
                return false;
            ...
            return base.OnKeyDown(args) || consumingText;
        }
        ...
        {
            if (!args.State.Keyboard.Keys.Any())
                EndConsumingText();
        ...
        protected override bool OnDrag(DragEventArgs args)
        {
            ...
            if (doubleClickWord != null)
            {
                ...
                if (getCharacterClosestTo(args.MousePosition) > doubleClickWord[1])
                {
                    selectionStart = doubleClickWord[0];
                    selectionEnd = findSeparatorIndex(text, getCharacterClosestTo(args.MousePosition) - 1, 1);
                    selectionEnd = selectionEnd >= 0 ? selectionEnd : text.Length;
                }
                else if (getCharacterClosestTo(args.MousePosition) < doubleClickWord[0])
                {
                    selectionStart = doubleClickWord[1];
                    selectionEnd = findSeparatorIndex(text, getCharacterClosestTo(args.MousePosition), -1);
            ...
            else
            {
                ...
                selectionEnd = getCharacterClosestTo(args.MousePosition);
                if (selectionLength > 0)
        ...
        protected override bool OnDoubleClick(DoubleClickEventArgs args)
        {
            ...
            {
                int hover = Math.Min(text.Length - 1, getCharacterClosestTo(args.MousePosition));
        ...
        protected override bool OnMouseDown(MouseDownEventArgs args)
        {
            ...
            selectionStart = selectionEnd = getCharacterClosestTo(args.MousePosition);
        ...
        protected override bool OnClick(ClickEventArgs args) => !ReadOnly;
