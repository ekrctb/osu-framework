...
namespace osu.Framework.Bindables
{
    ...
    /// Can only be retrieved via <see cref="LegacyBindable{T}.BeginLease"/>.
    /// </summary>
    /// <typeparam name="T"></typeparam>
    public class LeasedBindable<T> : LegacyBindable<T>
    {
        private readonly LegacyBindable<T> source;

        private readonly T valueBeforeLease;
        private readonly bool disabledBeforeLease;
        private readonly bool revertValueOnReturn;

        internal LeasedBindable([NotNull] LegacyBindable<T> source, bool revertValueOnReturn)
        {
            BindTo(source);
        ...
        /// <summary>
        /// End the lease on the source <see cref="LegacyBindable{T}"/>.
        /// </summary>
        public void Return()
        {
            if (source == null)
